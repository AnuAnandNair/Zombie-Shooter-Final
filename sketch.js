...
function setup() {
  ...
  //creating the player sprite
    player = createSprite(100, displayHeight-300, 50, 50);
...
function draw() {
  ...
  if(gameState === "fight"){
    ...
    if(zombieGroup.isTouching(player)){
      ...
      lose.play();
      for(var i=0;i<zombieGroup.length;i++){           
        if(zombieGroup.get(i).isTouching(player)){//if(zombieGroup[i].isTouching(player)){
          zombieGroup.get(i).destroy();//zombieGroup[i].destroy();     
          life=life-1;
        }  
      }
    }
  ...
  textSize(20);
  fill("white");
  text("Bullets = " + bullets,width-150,height/2-220);
  text("Score = " + score,width-150,height/2-190);
  text("Lives = " + life,width-150,height/2-250);
  ...
  //destroy zombie and player and display a message in gameState "won"
    ...
    textSize(85);
    fill("yellow");
    text("Congratulations!!!!",350,300);
    text("You Won",540,400);
    zombieGroup.destroyEach();
    player.destroy();
...
function enemy(){
  ...
  if(frameCount%100===0){
    ...
    zombie = createSprite(random(windowWidth/2, windowWidth),random(windowHeight/3,windowHeight-100),40,40);
